...
| Global settings | `<OS Config Dir>/code-trace-tree/settings.xml` (lazy; highlight colors and agent-skill notice, shared across projects and IDEs) |
...
Highlight line colors are **not** project data. They live in `<OS Config Dir>/code-trace-tree/settings.xml`, shared across projects and IDEs. The IDE creates this file on the first Advanced Settings save **or** the first agent-skill notice persist (not on read). After an Advanced Settings save it writes `signals/request_refresh_global_settings`. Agents do not need to write either file. The IDE does not watch `settings.xml`.
...
<settings>
  ...
  <agentSkill>
    <version>1</version>
    <noticeStatus>opened</noticeStatus>
  </agentSkill>
</settings>
```
